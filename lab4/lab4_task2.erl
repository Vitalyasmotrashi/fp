...
%% ============================================================
%% parent_children — модуль «родитель + N детей»
%%
%% Правила:
%%  * Дети: ждут сообщений.
%%      stop           -> завершиться нормально
%%      die            -> упасть с ошибкой
%%      любое другое   -> напечатать
%%  * Родитель: если ребёнок упал с ошибкой — перезапустить, напечатать сообщение.
%%  * Если родитель умирает — все дети тоже умирают (через link).
%% ============================================================

%% start(N) — запустить родителя и N детей, зарегистрировать родителя
start(N) ->
    Pid = spawn(fun() ->
